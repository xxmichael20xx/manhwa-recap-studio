<template>
  <div v-if="franchise" class="space-y-8">
    ...
    <div class="flex items-center justify-between">
      ...
      <span class="text-xs font-mono px-3 py-1 rounded bg-brand-crimson-500/10 text-brand-crimson-600 dark:text-brand-crimson-400 border border-brand-crimson-500/20 uppercase font-semibold">
        {{ franchise.folder }}
      </span>
    </div>
    ...
    <div class="bg-card border border-border rounded-2xl p-6 sm:p-8 relative overflow-hidden shadow-sm">
      <div class="space-y-4 max-w-3xl">
        ...
        <div class="space-y-2">
          ...
          <div class="bg-secondary/60 p-3 rounded-xl border border-border text-xs font-mono text-brand-cyan-600 dark:text-brand-cyan-400 break-all select-all">
    ...
    <div class="space-y-4">
      <h2 class="text-lg font-bold text-foreground flex items-center space-x-2">
        <Layers class="w-5 h-5 text-brand-crimson-500" />
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div 
          ...
          :key="ep.id"
          class="bg-card border border-border rounded-2xl p-6 space-y-6 hover:border-brand-crimson-500/30 transition-all shadow-sm"
        >
          <div class="flex items-start justify-between">
            ...
            <div v-if="ep.audit" class="text-right">
              <span class="text-xs font-mono font-bold px-2 py-1 rounded bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
          ...
          <div class="grid grid-cols-3 gap-3">
            <router-link 
              ...
              class="p-3 rounded-xl bg-secondary/40 border border-border hover:border-brand-crimson-500/50 text-center space-y-1 transition-all group"
            >
              <FileText class="w-4 h-4 mx-auto text-brand-crimson-500 group-hover:scale-110 transition-transform" />
            ...
            <router-link 
              ...
              class="p-3 rounded-xl bg-secondary/40 border border-border hover:border-brand-cyan-500/50 text-center space-y-1 transition-all group"
            >
              <Sparkles class="w-4 h-4 mx-auto text-brand-cyan-500 group-hover:scale-110 transition-transform" />
              <div class="text-[11px] font-bold text-foreground">Prompt Matrix</div>
              <div class="text-[9px] font-mono text-brand-cyan-600 dark:text-brand-cyan-400">22 Prompts</div>
            </router-link>

            <router-link 
              :to="`/tts/${franchise.folder}/${ep.id}`"
              class="p-3 rounded-xl bg-secondary/40 border border-border hover:border-purple-500/50 text-center space-y-1 transition-all group"
            >
              <Mic class="w-4 h-4 mx-auto text-purple-500 group-hover:scale-110 transition-transform" />
